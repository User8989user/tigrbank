...
import com.tigrbank.api.RestApiServer;
...
public class AppConfig {
        public static DIContainer createContainer() {
                ...
                container.register(ImportExportService.class, () -> new ImportExportService(exporters, importers,
                                container.resolve(BankAccountRepository.class),
                                container.resolve(CategoryRepository.class),
                                container.resolve(OperationRepository.class)));
                ...
                RestApiServer restServer = new RestApiServer(
                                ...
                                container.resolve(AnalyticsFacade.class));
                restServer.start(4567); // порт 4567
                // Реестр команд
                CommandRegistry registry = new CommandRegistry();

                // Команды для счетов
                registry.register("1",
                                new TimedCommand(new CreateAccountCommand(accountFacade, scanner), statisticsService));
                registry.register("2", new TimedCommand(new ListAccountsCommand(accountFacade), statisticsService));
                registry.register("3",
                                new TimedCommand(new UpdateAccountCommand(accountFacade, scanner), statisticsService));
                registry.register("4",
                                new TimedCommand(new DeleteAccountCommand(accountFacade, scanner), statisticsService));
                registry.register("5", new TimedCommand(new RecalculateBalanceCommand(accountFacade, scanner),
                                statisticsService));

                // Команды для категорий
                registry.register("6", new TimedCommand(new CreateCategoryCommand(categoryFacade, scanner),
                                statisticsService));
                registry.register("7", new TimedCommand(new ListCategoriesCommand(categoryFacade), statisticsService));
                registry.register("8", new TimedCommand(new UpdateCategoryCommand(categoryFacade, scanner),
                                statisticsService));
                registry.register("9", new TimedCommand(new DeleteCategoryCommand(categoryFacade, scanner),
                                statisticsService));

                // Команды для операций
                registry.register("10", new TimedCommand(new CreateOperationCommand(operationFacade, scanner),
                                statisticsService));
                registry.register("11",
                                new TimedCommand(new ListOperationsCommand(operationFacade), statisticsService));
                registry.register("12", new TimedCommand(new UpdateOperationCommand(operationFacade, scanner),
                                statisticsService));
                registry.register("13", new TimedCommand(new DeleteOperationCommand(operationFacade, scanner),
                                statisticsService));

                // Команда аналитики
                registry.register("14", new TimedCommand(new ShowAnalyticsCommand(analyticsFacade, scanner),
                                statisticsService));

                // Команды импорта/экспорта
                registry.register("15", new TimedCommand(new ExportDataCommand(importExportService, scanner),
                                statisticsService));
                registry.register("16", new TimedCommand(new ImportDataCommand(importExportService, scanner),
                                statisticsService));
