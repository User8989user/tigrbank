...
public class JsonImporter extends DataImporterTemplate {
    ...
    protected RawData readRawData(String basePath) throws IOException {
        // Читаем JSON-файлы в сырые списки Map
        List<Map<String, Object>> accountsRaw = mapper.readValue(
                new File(basePath + "_accounts.json"),
                mapper.getTypeFactory().constructCollectionType(List.class, Map.class));
        List<Map<String, Object>> categoriesRaw = mapper.readValue(
                new File(basePath + "_categories.json"),
                mapper.getTypeFactory().constructCollectionType(List.class, Map.class));
        List<Map<String, Object>> operationsRaw = mapper.readValue(
                new File(basePath + "_operations.json"),
                mapper.getTypeFactory().constructCollectionType(List.class, Map.class));

        // Преобразуем Map<String,Object> в Map<String,String>
        return new RawData(
